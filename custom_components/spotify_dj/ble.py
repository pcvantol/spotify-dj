...
async def async_discover_devices(hass: HomeAssistant) -> dict[str, str]:
    """Return visible SpotifyDJ BLE setup devices by address."""
    try:
        from homeassistant.components import bluetooth

        infos = bluetooth.async_discovered_service_info(hass, connectable=True)
    except Exception:  # noqa: BLE001
        _LOGGER.debug("SpotifyDJ BLE discovery unavailable", exc_info=True)
        return {}
    ...
    for info in infos or []:
        ...
        if name.startswith(BLE_NAME_PREFIX) or BLE_SERVICE_UUID in uuids:
            devices[address] = name or f"SpotifyDJ {address}"
    return devices
...
async def async_provision_wifi(
    ...
    """Write WiFi credentials to a SpotifyDJ BLE device and read its status."""
    if not address:
        raise RuntimeError("No SpotifyDJ BLE device selected")
    if not ssid:
...
async def _disconnect_client(client: Any) -> None:
    try:
        ...
        _LOGGER.debug("SpotifyDJ BLE disconnect failed", exc_info=True)
