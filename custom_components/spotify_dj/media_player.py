...
async def async_setup_entry(
    ...
    async_add_entities([SpotifyDJPlaybackProxyMediaPlayer(runtime, hass)])


class SpotifyDJPlaybackProxyMediaPlayer(MediaPlayerEntity):
    """Backend playback session controlled through SpotifyDJ's HA integration."""

    _attr_has_entity_name = True
    _attr_translation_key = "playback_proxy"
    _attr_unique_id = "spotifydj_playback_proxy"
    ...
    async def _backend_command(
        ...
        *,
        ...
        try:
            ...
            _LOGGER.warning("SpotifyDJ playback backend unavailable: %s", exc)
            raise
        except Exception as exc:  # noqa: BLE001
            self.runtime.update(last_error=str(exc))
            _LOGGER.warning("SpotifyDJ playback proxy command failed: %s", exc)
            raise
