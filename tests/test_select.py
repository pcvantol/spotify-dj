...
class SpotifyDJSelectTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        install_select_stubs()
        cls.select = importlib.import_module("custom_components.spotify_dj.select")
    ...
    def test_theme_and_log_level_have_initial_defaults(self) -> None:
        ...
        theme = self.select.SpotifyDJCommandSelect(runtime, object(), "theme", "theme", "theme", ["light", "dark", "auto"])
        log_level = self.select.SpotifyDJCommandSelect(runtime, object(), "log_level", "log_level", "log_level", ["debug", "info", "warning", "error"])
        language = self.select.SpotifyDJCommandSelect(runtime, object(), "language", "language", "language", ["en", "nl"])
